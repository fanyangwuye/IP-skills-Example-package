...
```text
使用已安装的 IP skills 跑完整流程，不要跳过角色图、场景图、故事板/拍摄表、参考策略、I2V、尾帧衔接和连续性检查。
```
...
```text
使用 IP skills 的完整流程：文案/资产包 -> 角色图 -> 场景图 -> 故事板/拍摄表 -> 参考策略 -> I2V -> 尾帧衔接 -> 下一段视频连续性检查。
```
...
5. 配置 API key 到环境变量，不要写入仓库。生成产物统一设置 `IP_SKILLS_OUTPUT_ROOT`，优先放到 E/D/G 盘项目目录，不要默认写入 `C:\Users`。
6. 先跑离线测试，再做 live 小样。

通用调用句：

```text
使用本仓库 IP skills 跑完整 IP 生产流程，不要跳过角色设定、场景参考、故事板/拍摄表、参考策略、视频生成、尾帧衔接和连续性检查。
```
