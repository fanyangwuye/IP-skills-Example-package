...
Offline mode never calls a provider and now returns a reviewable prompt pack under `raw_response`. Mock mode is for tests. The live LLM engine is guarded; after double approval it currently builds a dry-run provider request with `network_call_allowed=false` plus provider boundary blockers, and still does not make provider calls. Prompt packs now include format-specific writing templates and few-shot output shapes as stronger control artifacts, not proof that final creative prose has been generated.
